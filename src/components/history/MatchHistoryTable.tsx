            ...
            <div className="hidden lg:block min-w-[1200px] overflow-x-auto">
                ...
                {matches.map(match => (
                    <div key={match.id} className="grid grid-cols-10 items-center text-center bg-gray-200 rounded-lg hover:bg-lime-50 transition mb-2" style={{ minWidth: '1200px' }}>
                        ...
                        <div className="col-span-2 py-4 font-semibold text-[#000000]">{match.clubName || 'Không xác định'}</div>
                    ...
                    <div key={match.id} className="bg-gray-200 rounded-lg p-4 hover:bg-lime-50 transition">
                        <div className="flex justify-between items-start mb-3">
                            <div className="text-center flex-1">
                                <div className="text-sm font-semibold text-[#000000] mb-1">THỜI GIAN</div>
                                <div className="text-sm font-bold text-black">{match.time}</div>
                            </div>
                            <div className="text-center flex-1">
                                <div className="text-sm font-semibold text-[#000000] mb-1">CHI NHÁNH</div>
                                <div className="text-sm text-[#000000]">{match.clubName || 'Không xác định'}</div>
                            </div>
                            <div className="text-center flex-1">
                                <div className="text-sm font-semibold text-[#000000] mb-1">THỂ THỨC</div>
                        ...
                        <div className="grid grid-cols-2 gap-2 mb-3">
                            <div className="text-center">
                                <div className="text-xs font-bold text-[#000000] mb-1">ĐỘI THẮNG</div>
                                {match.winningTeamMembers.map((name: string, idx: number) => (
                                    <div key={idx} className="text-xs text-[#000000]">{name}</div>
                                ))}
                            </div>
                            <div className="text-center flex flex-col justify-center">
                                <div className="text-xs font-bold text-[#000000] mb-1">TỶ SỐ</div>
