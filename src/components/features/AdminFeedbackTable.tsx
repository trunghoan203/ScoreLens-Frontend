...
export function AdminFeedbackTable() {
    ...
    useEffect(() => {
        ...
        adminFeedbackService.getAllFeedbacks()
            ...
            .catch(() => {
                toast.error(t('adminFeedbacks.cannotLoadFeedbacks'));
                setFeedbacks([]);
                setLoading(false);
            });
    }, [t]);
    ...
    const filteredFeedbacks = feedbacks.filter((fb) => {
        // Admin can only see: managerP, adminP, superadminP, resolved
        if (fb.status !== 'managerP' && fb.status !== 'adminP' && fb.status !== 'superadminP' && fb.status !== 'resolved') {
            return false;
        }
        ...
        if (statusFilter === 'pending') {
            matchesStatus = fb.status === 'adminP';
        } else if (statusFilter === 'resolved') {
            matchesStatus = fb.status === 'resolved';
        } else if (statusFilter === 'all') {
            matchesStatus = fb.status === 'managerP' || fb.status === 'adminP' || fb.status === 'superadminP' || fb.status === 'resolved';
        }
    ...
    const getStatusText = (status: string) => {
        switch (status) {
            case 'resolved': return t('adminFeedbacks.statusResolved');
            case 'managerP': return t('adminFeedbacks.statusManagerP');
            case 'adminP': return t('adminFeedbacks.statusAdminP');
            case 'superadminP': return t('adminFeedbacks.statusSuperadminP');
            default: return t('adminFeedbacks.statusUnknown');
        }
    };
    ...
    return (
        <div className="space-y-4 sm:space-y-6">
            <div className="backdrop-blur-md border-lime-400 bg-white/60 border border-gray-200 rounded-2xl shadow-lg px-4 sm:px-6 py-4 sm:py-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-6 transition-all duration-300">
                ...
                <div className="flex gap-3 w-full sm:w-auto">
                    <div className="relative w-full sm:w-50">
                        ...
                        >
                            <option value="all">{t('adminFeedbacks.allStatus')}</option>
                            <option value="pending">{t('adminFeedbacks.pendingStatus')}</option>
                            <option value="resolved">{t('adminFeedbacks.resolvedStatus')}</option>
                        </select>
                        <Image
                ...
                <div className="hidden lg:block overflow-x-auto">
                    <div className="space-y-2 rounded-lg min-w-[800px]">
                        <div className="grid grid-cols-12 bg-black text-white font-semibold text-center">
                            <div className="col-span-3 py-3 text-sm xl:text-base">{t('adminFeedbacks.brandColumn')}</div>
                            <div className="col-span-3 py-3 text-sm xl:text-base">{t('adminFeedbacks.branchColumn')}</div>
                            <div className="col-span-3 py-3 text-sm xl:text-base">{t('adminFeedbacks.dateColumn')}</div>
                            <div className="col-span-3 py-3 text-sm xl:text-base">{t('adminFeedbacks.statusColumn')}</div>
                        </div>
                        {loading ? (
                            <div className="py-6 sm:py-8 text-center text-gray-500 text-sm sm:text-base">{t('adminFeedbacks.loading')}</div>
                            ...
                            displayedFeedbacks.map((fb) => (
                                <div
                                    ...
                                    onClick={() => router.push(`/admin/feedback/${fb.feedbackId}`)}
                                >
                                    <div className="col-span-3 py-4 font-semibold text-black text-sm xl:text-base px-2">
                                        {fb.clubInfo?.brandName || t('adminFeedbacks.unknown')}
                                    </div>
                            ...
                            <div className="py-6 sm:py-8 text-center text-gray-500 text-sm sm:text-base">{t('adminFeedbacks.noFeedbacksFound')}</div>
                        )}
                    </div>
                </div>
                            ...
                            >
                                ...
                                <div className="space-y-2">
                                    <div className="flex items-center">
                                        <span className="text-gray-500 text-xs font-medium w-8">{t('adminFeedbacks.dateLabel')}</span>
                                ...
                                <div className="mt-3 pt-3 border-t border-gray-100">
                                    <div className="flex justify-end">
                                        <span className="text-lime-600 text-xs font-medium">{t('adminFeedbacks.clickToViewDetails')}</span>
                                    </div>
                                </div>
                            </div>
                        ))
            ...
            <div className="mt-3 sm:mt-4 text-center text-gray-400 italic text-xs sm:text-sm">
                {t('adminFeedbacks.showingResults').replace('{start}', (startIndex + 1).toString()).replace('{end}', Math.min(endIndex, sortedFeedbacks.length).toString()).replace('{total}', sortedFeedbacks.length.toString())}
            </div>
        </div>
    );
}
