...
export default function FeedbackDetailPage() {
        ...
        <div className="bg-white border border-lime-300 rounded-2xl shadow w-full max-w-md p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm text-gray-900">
            <div className="space-y-1">
              <p><span className="font-semibold">Chi nhánh:</span> {feedback.branch}</p>
              <p><span className="font-semibold">Bàn:</span> {feedback.table}</p>
              <p><span className="font-semibold">Ngày:</span> {feedback.date}</p>
            </div>
            <div className="space-y-1">
  <p>
    ...
    {loadingCamera ? (
      ...
      <span className="text-green-600 font-semibold">{cameraConfidence}%</span>
    )}
  </p>
  ...
  <p><span className="font-semibold">Vấn đề:</span> {feedback.issue}</p>
</div>
          </div>

          <div>
            <label className="block mb-1 font-semibold text-sm text-gray-700">Ghi chú:</label>
            <textarea
              value={note}
