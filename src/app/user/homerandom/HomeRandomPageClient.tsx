// src/app/user/homerandom/HomeRandomPageClient.tsx
...
export function HomeRandomPageClient() {
    ...
    const qrJoinUrl = process.env.NEXT_PUBLIC_BASE_URL
        ? `${process.env.NEXT_PUBLIC_BASE_URL}/user/guestlogin?table=${tableNumber}&room=${roomCode}`
        : `https://example.com/user/guestlogin?table=${tableNumber}&room=${roomCode}`;
            ...
            <div className="flex-1 flex flex-col items-center text-center space-y-8 py-10 w-full max-w-md sm:max-w-lg md:max-w-xl lg:max-w-2xl mx-auto">
                ...
                <div className="space-y-1">
                    ...
                    <div className="p-2 bg-white border rounded-md inline-block">
                        {/* Chỉ render QR code khi có URL */}
                        {roomCode && <QRCodeCanvas value={qrJoinUrl} size={180} />}
                    </div>
                </div>
            </div>
